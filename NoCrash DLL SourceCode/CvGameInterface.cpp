		...
		else if(pHeadSelectedUnit->airRange() > 0) //other ranged units
		{
			...
			for (iDX = -(iRange); iDX <= iRange; iDX++)
			{
				for (iDY = -(iRange); iDY <= iRange; iDY++)
				{
					...
					if (plotDistance(pHeadSelectedUnit->plot(), pTargetPlot) > iRange)
					{
						continue;
					}

					if (!pHeadSelectedUnit->plot()->canSeePlot(pTargetPlot, pHeadSelectedUnit->getTeam(), iRange))
					{
						continue;
					}
		...
		if (pCity->getOwnerINLINE() == getActivePlayer())
		{
			...
			for (int iI = 0; iI < GC.getNumHurryInfos(); iI++)
			{
				if (pCity->canHurry((HurryTypes)iI))
				{
					...
					int iHurryCulture = pCity->hurryCulture((HurryTypes)iI);
					if (iHurryCulture > 0)
					{
						szBuffer += gDLL->getText("TXT_KEY_HURRY_PRODUCTION_CULTURE", iHurryCulture);
					}
